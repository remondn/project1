...
int main(int argc, char** argv){
    ...
    std::vector<point>::iterator currentGoal = path.begin();

    drive_msg_stamped.drive.steering_angle = 0;
    drive_msg_stamped.drive.speed = MAX_SPEED;
    ...
    while(ros::ok()){
        ...
        */
        ...
        drive_msg_stamped.drive.steering_angle += pid_ctrl.get_control(car_pose, (*currentGoal));
        drive_msg_stamped.drive.speed = MAX_SPEED;  // Fast & Furious

        // Step 2 : Publish the new data
        car_ctrl_pub.publish(drive_msg_stamped);

        // Step 3 : Check if we reached the point
        if(distance(car_pose, (*currentGoal)) < 0.2)
        {
            //DEBUG
            std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
        ...
        //Step 4 : Check if we reached our final destination
        if(currentGoal == path.end())
        {
            ros::shutdown();    // Bye Paul Walker
